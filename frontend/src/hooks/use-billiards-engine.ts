...
export function useBilliardsEngine(
  ...
  useEffect(() => { callbackRef.current = onShotResolved })

  const onRollingChangeRef = useRef(onRollingChange)
  useEffect(() => { onRollingChangeRef.current = onRollingChange })

  const activeEffectsRef = useRef<Set<BuffEffect>>(new Set())
  useEffect(() => { activeEffectsRef.current = activeEffects ?? new Set() })
